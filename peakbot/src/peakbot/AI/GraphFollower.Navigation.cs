...
namespace Peak.BotClone
{
    internal partial class GraphFollower
    {
        ...
        const float CONE_TIGHT_DOT = 0f;
        const float CONE_WIDE_DOT  = -0.70710678f;

        // Greedy scoring weights
        const float W_ALIGN   = 0.6f;  // alignment with vector to player
        const float W_DIST    = 0.3f;  // relative distance improvement to player
        const float W_SMOOTH  = 0.1f;  // heading continuity vs last step
        const float TIE_NOISE = 0.01f; // small randomness to break ties

        // State to discourage immediate backtrack and promote smooth motion
        NavPoint? previousNode;
        Vector3   lastGraphStep;

        // Log throttling during fallback churn
        NavMeshPathStatus? _lastPathStatusForLog;
        bool               _lastHasPathForLog;
        float              _nextFallbackLogAt;
        ...
        List<NavPoint> BuildCandidates(NavPoint from, Vector3 toPlayerNorm, float minDot, bool avoidBacktrack)
        {
